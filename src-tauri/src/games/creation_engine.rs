...
pub const CREATION_ENGINE_ROOT_DIRS: &[&str] = &[
    "Data",
    "SKSE",
    "F4SE",
    "SFSE",
...
fn extender_preflight(install_path: &Path, title: CreationTitle) -> Vec<String> {
    let found = title.extender_names.iter().any(|n| install_path.join(n).is_file())
...
fn obr_data_dir(install_path: &Path) -> Option<PathBuf> {
    ...
    if data.is_dir() || project.join("Content").is_dir() {
        Some(data)
    } else if install_path.join("Content").join("Dev").join("ObvData").join("Data").is_dir() {
        Some(
            install_path
                .join("Content")
                .join("Dev")
...
fn write_plugins_txt(
    ...
    write_plugins_file(&path, data_dir, title.required_masters, title.asterisk_enabled)
}

fn plugins_txt_path(install_path: &Path, title: CreationTitle) -> Option<PathBuf> {
    if title.plugins_in_documents {
        if let Some(dir) = documents_game_dir(install_path, title.steam_app_id, title.documents_game)
        {
...
fn write_plugins_file(
    ...
    for master in required_masters {
        if found.iter().any(|f| f.eq_ignore_ascii_case(master))
            || data_dir.join(master).is_file()
        {
...
fn local_appdata_game_dir(
    ...
    #[cfg(windows)]
    {
        if let Some(local) = std::env::var_os("LOCALAPPDATA").map(PathBuf::from) {
            return Some(local.join(game_folder));
        }
    }
    proton_appdata_local(install_path, steam_app_id).map(|p| p.join(game_folder))
}
...
fn documents_game_dir(
    ...
    #[cfg(windows)]
    {
        if let Some(docs) = directories::UserDirs::new().and_then(|u| u.document_dir().map(|d| d.to_path_buf())) {
            return Some(docs.join("My Games").join(game_folder));
        }
    }
...
fn proton_appdata_local(install_path: &Path, steam_app_id: &str) -> Option<PathBuf> {
    let steamapps = find_steamapps_dir(install_path)?;
    Some(
        steamapps
            .join("compatdata")
